...
def test_key_fact_create() -> None:
    f = KeyFact.create(tenant_id=uuid4(), participant_identifier="+971500000000", key="  Name  ", value="  Sara  ")
    assert f.is_new
    assert f.key == "name"
    assert f.value == "Sara"
...
def test_key_fact_update_value() -> None:
    f = KeyFact.create(tenant_id=uuid4(), participant_identifier="user@test.com", key="language", value="English")
