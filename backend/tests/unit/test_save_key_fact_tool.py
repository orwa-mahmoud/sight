...
async def test_save_key_fact_creates_new(client: None) -> None:
    ...
    async with async_session_factory() as session:
        result = await run_save_key_fact(
            ...
            tenant_id=t.id,
            participant_identifier="+971500000000",
            session=session,
        )
...
async def test_save_key_fact_updates_existing(client: None) -> None:
    async with async_session_factory() as session:
        ...
        await uow.tenants.save(t)
        await uow.flush()
        f = KeyFact.create(tenant_id=t.id, participant_identifier="p1", key="name", value="Old")
...
async def test_remove_key_fact_removes(client: None) -> None:
    async with async_session_factory() as session:
        ...
        await uow.tenants.save(t)
        await uow.flush()
        f = KeyFact.create(tenant_id=t.id, participant_identifier="p1", key="name", value="Sara")
        await uow.key_facts.save(f)
        await uow.commit()
